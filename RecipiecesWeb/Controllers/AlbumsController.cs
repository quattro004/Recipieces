...
using RecipiecesWeb.Filters;
using RecipiecesWeb.Helpers;
...
namespace RecipiecesWeb.Controllers
{
    ...
    public class AlbumsController : ControllerBase
    {
        ...
        public AlbumsController(IDataService<Album<MediaContent>> albumService, ILogger<AlbumsController> logger,
            IOptionsMonitor<ApiOptions> optionsAccessor)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            _albumService = albumService ?? throw new ArgumentNullException(nameof(albumService));
            _apiOptions = optionsAccessor?.CurrentValue ?? throw new ArgumentNullException(nameof(optionsAccessor));
            //
            // Save physical files to a path provided by configuration:
            if (!Directory.Exists(_apiOptions.AlbumContentPath))
            {
                Directory.CreateDirectory(_apiOptions.AlbumContentPath);
            }
        ...
        public async Task<IActionResult> CreateAsync(Album<MediaContent> dataObject)
        {
            ...
            return CreatedAtRoute("GetAlbum", new { id = newDataObject.Id }, newDataObject);
        }
                ...
                if (hasContentDispositionHeader)
                {
                    ...
                    else
                    {
                        ...
                        var streamedFileContent = await FileHelpers.ProcessStreamedFile(section, contentDisposition,
                            ModelState,  _permittedExtensions, _apiOptions.FileSizeLimit);

                        if (!ModelState.IsValid)
                        {
                            return BadRequest(ModelState);
                        }
                        var filePath = Path.Combine(_apiOptions.AlbumContentPath, trustedFileNameForFileStorage);
                        ...
                        using (var targetStream = System.IO.File.Create(filePath))
                        {
                            ...
                            _logger.LogInformation(
                                ...
                                $"'{_apiOptions.AlbumContentPath}' as {trustedFileNameForFileStorage}", 
                                trustedFileNameForDisplay, _apiOptions.AlbumContentPath, 
